//! 共享 HTTP 客户端、搜索结果解析与 HTML 文本提取。

use std::sync::LazyLock;

/// 全局 HTTP 客户端（连接池复用，避免每次搜索重建 + TCP 握手开销）
pub(super) static CLIENT: LazyLock<reqwest::Client> = LazyLock::new(|| {
...
pub(super) fn parse_bing_results(html: &str, max: usize) -> Vec<(String, String, String)> {
    // 策略 1 + 2: 先找所有可能的结果条目
...
pub(super) fn parse_ddg_lite_results(html: &str, max: usize) -> Vec<(String, String, String)> {
    let mut results = Vec::new();
...
pub(super) async fn try_ddg_async(url: &str, variant: &str, max: usize) -> Option<String> {
