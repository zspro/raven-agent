...
use raven_types::{ChatResponse, Message, ModelInfo, ProviderConfig, ProviderFeatures, ProviderVerification, StreamEvent, ToolSchema};
...
impl ProviderKind {
    ...
    pub fn detect(name: &str, base_url: &str) -> Self {
        ...
        } else if hay.contains("gemini") || hay.contains("googleapis") || hay.contains("generativelanguage") {
            ProviderKind::Gemini
        } else {
            ProviderKind::OpenAI
        }
...
pub trait ProviderClient: Send + Sync {
    ...
    async fn chat_stream(&self, model: &str, messages: &[Message], tools: Option<&[ToolSchema]>) -> Result<tokio::sync::mpsc::Receiver<StreamEvent>, raven_types::AgentError>;
...
impl Router {
    ...
    /// 注册提供商
    pub async fn register_provider(&self, config: ProviderConfig) -> Result<(), raven_types::AgentError> {
        ...
        providers.push(ProviderEntry { name, config: client_config, client });

        Ok(())
    }

    /// 注册默认提供商（从主配置）
    pub async fn register_default(&self, api_key: Option<String>, base_url: Option<String>, model: String) -> Result<(), raven_types::AgentError> {
    ...
    /// 发送聊天请求
    pub async fn chat(&self, model_id: &str, messages: &[Message], tools: Option<&[ToolSchema]>) -> Result<ChatResponse, raven_types::AgentError> {
        let (client, model) = self.resolve(model_id).await?;
        client.chat(&model, messages, tools).await
    }

    /// 流式聊天
    pub async fn chat_stream(&self, model_id: &str, messages: &[Message], tools: Option<&[ToolSchema]>) -> Result<tokio::sync::mpsc::Receiver<StreamEvent>, raven_types::AgentError> {
        let (client, model) = self.resolve(model_id).await?;
    ...
    /// 验证指定提供商
    pub async fn verify_provider(&self, name: &str) -> Result<ProviderVerification, raven_types::AgentError> {
        let providers = self.providers.read().await;
        let entry = providers.iter()
            .find(|p| p.name == name)
            .ok_or_else(|| raven_types::AgentError::config(
                format!("未知的提供商: {}", name),
    ...
    /// 解析模型ID
    async fn resolve(&self, model_id: &str) -> Result<(Box<dyn ProviderClient>, String), raven_types::AgentError> {
        let providers = self.providers.read().await;
        ...
        if let Some(pos) = model_id.find('/') {
            ...
            let entry = providers.iter()
                ...
                .ok_or_else(|| raven_types::AgentError::config(
                    ...
                    format!("可用提供商: {}", providers.iter().map(|p| p.name.as_str()).collect::<Vec<_>>().join(", ")),
                ))?;
    ...
    async fn test_features(entry: &ProviderEntry) -> ProviderFeatures {
        ...
        if let Ok(mut rx) = entry.client.chat_stream(
            entry.config.default_model.as_deref().unwrap_or("gpt-4o"),
            &messages,
            None,
        ).await {
            // 等待第一个事件
            if let Some(_event) = tokio::time::timeout(
        ...
        if let Ok(_resp) = entry.client.chat(
            entry.config.default_model.as_deref().unwrap_or("gpt-4o"),
            &messages,
            Some(&tools),
        ).await {
    ...
    fn infer_name(url: &str) -> String {
        ...
        if url.contains("openai") { return "openai".to_string(); }
        if url.contains("anthropic") || url.contains("claude") { return "anthropic".to_string(); }
        if url.contains("generativelanguage") || url.contains("gemini") || url.contains("googleapis") { return "gemini".to_string(); }
        if url.contains("deepseek") { return "deepseek".to_string(); }
        if url.contains("groq") { return "groq".to_string(); }
        if url.contains("together") { return "together".to_string(); }
        if url.contains("ollama") { return "ollama".to_string(); }
        "custom".to_string()
    }
}
